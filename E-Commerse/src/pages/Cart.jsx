...
import { get, del, getRenderURL } from "../services/api";
...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      const userId = buyer?._id || supplier?._id;
      ...
      try {
        const endpoint = `/cart/${userId}?userType=${userType}`;
        console.log("Fetching cart from:", getRenderURL() + endpoint);
        const data = await get(endpoint); // ✅ centralized api.js handles base URL
  ...
  const handleRemove = async (id) => {
    try {
      ...
      console.log("Deleting cart item via:", getRenderURL() + endpoint);
      await del(endpoint);
